<!--- 
/** 
* Copyright 2004-2008 massimocorner.com
...
* A stateless CFC that support different kind of file I/O tasks. 
  ...
  ColdFusion 7 or above required
* @output      supressed 
* @author      Massimo Foti (massimo@massimocorner.com)
* @version     1.5.1, 2008-12-18
 */
--->
<cfcomponent output="no" hint="
	...
	* @return      boolean
	...
	<!--- 
	/** 
	* Return system drives (or roots)
	* @access      public
	* @output      suppressed 
	* @return      query
	 */
	--->
	<cffunction name="getDrives" access="public" output="false" returntype="query" hint="Return system drives (or roots)">
		<cfset var drives="">
		<cfset var i="">
		<cfset var result=QueryNew("name")>
		<cfset drives=urlToJavaFile("").listRoots()>
	...
	* @param       destination (string)          Required. Absolute directory path of a directory where the contents will be moved
	 */
	--->
	<cffunction name="copyDirectory" access="public" output="false" returntype="void" hint="Copy a directory, including all its contents">
	...
	* @exception   tmt_file_io
	 */
	--->
	<cffunction name="createDirectory" access="public" output="false" returntype="void" hint="Creates a directory, including any necessary but nonexistent parent directories. 
Note that if this operation fails it may have succeeded in creating some of the necessary parent directories">
		<cfargument name="directoryPath" type="string" required="true" hint="Absolute directory path">
	...
	* @param       throwIfMissing (boolean)      Optional. Default to false. Raise an exception if the directory doesn’t exists. Default to false
	 */
	--->
	<cffunction name="deleteDirectory" access="public" output="false" returntype="void" hint="Delete a directory, including all its contents">
		<cfargument name="directoryPath" type="string" required="true" hint="Absolute directory path">
	...
	* @param       destination (string)          Required. Absolute directory path of a directory where the contents will be moved
	 */
	--->
	<cffunction name="moveDirectory" access="public" output="false" returntype="void" hint="Move a directory, including all its contents">
		<cfargument name="source" type="string" required="true" hint="Absolute directory path">
		<cfargument name="destination" type="string" required="true" hint="Absolute directory path of a directory where the contents will be moved">
		<cfscript>
	...
	* @exception   tmt_file_io
	 */
	--->
	<cffunction name="renameDirectory" access="public" output="false" returntype="void" hint="Rename a directory">
		<cfargument name="directoryPath" type="string" required="true" hint="Absolute directory path">
	...
	* @exception   tmt_file_io
	 */
	--->
	<cffunction name="appendFile" access="public" output="false" returntype="void" hint="Appends text to a file">
	...
	* @exception   tmt_file_io
	 */
	--->
	<cffunction name="copyFile" access="public" output="false" returntype="void" hint="Copy a file">
	...
	* @exception   tmt_file_io
	 */
	--->
	<cffunction name="deleteFile" access="public" output="false" returntype="void" hint="Delete a file">
		<cfargument name="filePath" type="string" required="true" hint="Absolute file path">
	...
	* @return      string
	 */
	--->	
	<cffunction name="getFileMimeType" access="public" output="false" returntype="string" hint="Returns the mime type of the file">
		<cfargument name="filePath" type="string" required="true" hint="Absolute file path">
	...
	* @exception   tmt_file_io
	 */
	--->
	<cffunction name="renameFile" access="public" output="false" returntype="void" hint="Rename a file">
		<cfargument name="filePath" type="string" required="true" hint="Absolute file path">
	...
	* @exception   tmt_file_io
	 */
	--->
	<cffunction name="writeFile" access="public" output="false" returntype="void" hint="Write to a text file">
		<cfargument name="filePath" type="string" required="true" hint="Absolute file path">
		<cfargument name="fileContent" type="string" required="true" hint="Content of the file to be created">
		<cfargument name="throwIfMissing" type="boolean" required="false" default="false" hint="Raise an exception if the file doesn’t exists. Default to false">
		<cfargument name="fileCharset" type="string" required="false" default="#variables.charset#" hint="Charset used. Default to a predefined value">
	...
	* @access      public
	* @output      suppressed 
	* @param       fileField (string)            Required. The name of the form file field
	* @param       destination (string)          Optional. Default to current folder. Absolute directory path of a directory where the file will be stored.
	* @param       maxsize (numeric)             Optional. Default to 100. Maximum file size allowed (kb).
	* @param       mimetypes (string)            Optional. Default to image/*. Comma delimited list of mime types allowed.
	* @param       nameconflict (string)         Optional. Default to makeunique. Action to take if filename is the same as that of a file in the directory.
	* @exception   tmt_file_io	
	* @return      string
	 */
	--->
	...
	">
		...
		<cfsavecontent variable="exceptionNoSave">
			...
			<ol>
				<li>The MIME type of the uploaded file was not accepted by the server. Please verify that you are uploading a file of the appropriate type</li>
				<cfif arguments.nameconflict EQ "Error">
					<li>A file with the same name already exist on the server</li>
				</cfif>
				<li>The application doesn't have the correct permissions on the server</li>
			</ol>
		</cfsavecontent>
		...
		<cfif IsDefined("uploadResults.fileWasSaved")>
			...
			<!--- Remove unsafe chars --->
			<cfset safeFullServerPath = fixFilename(fullServerPath)>
			<!--- Store the name of the file inside  the form variable --->
			<cfset form[arguments.filefield] = GetFileFromPath(safeFullServerPath)>
		<cfelse>
	...
	<!---
	Rename a file removing any reserved characters from a filename, replace any spaces with underscores and return the new file path
	* @param       filePath (string)             Required. Absolute file path
	* @return      string 
	--->
	<cffunction name="fixFilename" access="public" output="false" returntype="string" hint="Rename a file removing any reserved characters from a filename, replace any spaces with underscores and return the new file path">
		<cfargument name="filePath" type="string" required="true" hint="Absolute file path">
		<cfset var oldName = GetFileFromPath(arguments.filePath)>
		<cfset var pattern = "[" & "'" & '"' & "##" & "/\\%&`@~!,:;=<>\+\*\?\[\]\^\$\(\)\{\}\|]">
		<cfset var newName = ReReplace(oldName, pattern, "", "all")>
		<cfset newName = Replace(newName, " ", "_", "all")>
		<cfset renameFile(arguments.filePath, newName)>
		<cfreturn GetDirectoryFromPath(arguments.filePath) & newName>
	</cffunction>
